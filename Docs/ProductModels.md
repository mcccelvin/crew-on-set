...
| --- | --- | --- | --- |
| 1 | Artisan vase | Assets/Product/FlowerBase (1).fbx | 1 unit tall |
| 2 | Goke | Coke.fbx | 0.35 units tall; label faces local -Z |
| 3 | Lambormini | Car.fbx | Roof = 70% of standing player height |
| 4 | Kape Kultura | Artisan Coffee Brand.fbx | 0.45 units tall; label faces local -Z |
...
Level 1 now uses the supplied `FlowerBase (1).fbx`, which has two renderers. The catalog retains selection collision and the recording subject marker.
...
Level 1's Cube card now displays Table and places Assets/Product/TableForFlower.fbx at 1 metre tall. The existing Cube wrapper identifier is retained for tutorial compatibility; beginner instructions refer to the table. Furniture references live in ProductModels.asset.

The Level 1 table rotates 180 degrees before bounds normalization: the two curved legs frame the local -Z filming side and the third leg is behind. Height remains 1 metre with its bottom grounded.
