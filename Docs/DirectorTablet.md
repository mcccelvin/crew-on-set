...
ProductModels now displays the shared list as Backdrops And Interiors. Each entry includes world-space Position X/Y/Z offsets applied after centring and grounding, plus Model, Size, Scale and Rotation. The Coffee Interior entry supplies the existing CHOOSE SET > COFFEE INTERIOR option; its Coffee Interior Entry flag distinguishes it from backdrop variants. Active Green Screen selects ordinary backdrops only, falling back to the first ordinary entry if it points at an interior. Existing set prices, practice behavior and OwnedInterior.0/1/2 purchase keys are retained. Edit the list outside Play Mode and recreate the set. Coffee Interior's former standalone Model/Scale/Rotation fields are hidden compatibility fallbacks.

Select Assets/Resources/ProductModels.asset and expand Green Screens to edit Model, Size (overall multiplier), Scale (local X/Y/Z multipliers), and Rotation (degrees relative to the stage). Active Green Screen selects the zero-based list entry. Size 1, Scale (1,1,1), Rotation (0,0,0) preserve the existing size and orientation, including prefab and studio scaling. Settings apply to previews, purchases and practice walls. Edit outside Play Mode and recreate the backdrop to apply. Replacement models need a paintable renderer named Screen for existing grounding/color controls; mesh colliders are added where missing. Empty catalog entries fall back to the scene wallPrefab.
