...
public partial class AlmanacManager : MonoBehaviour
{
    ...
    public void UnlockLevel3Equipment()
    {
        ...
        UnlockKnowledge("camera_white_balance");
        PlayerPrefs.Save();
    }
    ...
    public void UnlockLevel4Knowledge()
    {
        ...
        EnsureEquipmentAndTechniqueEntries();
        RemoveLegacyKnowledgeEntries();
        UnlockKnowledge("actor_megaphone");
        ...
        UnlockKnowledge("camera_exposure");
        PlayerPrefs.Save();
    }
    ...
    private void EnsureLevel3KnowledgeEntries()
    {
        ...
        AddKnowledgeEntry("hiring_and_posing_actors", "TECHNIQUE - DIRECTING ACTORS", "Use the tablet to hire and place one actor. Buy and equip the megaphone; click the actor to select. X: Wave greeting. C: Action with coffee. Aim at a chair and click: Sitting. O: stop; T: reposition; R: turn. Film each performance separately. Keep actor and coffee visible. Any actor tier can complete the story.", "Technique", 4, 0);
        AddKnowledgeEntry("automotive_staging", "LEVEL 3 - AUTOMOTIVE STAGING", "Vehicle commercials require a readable silhouette, controlled reflections, and deliberate negative space.\n\n- Click the approved car card, move it over the stage, then click to place it.\n- Leave open space around the vehicle and show its important front or side shape.\n- Use the Soft Light across body panels to reveal their form.\n- Use the NONY FX Camera grid to place the vehicle deliberately instead of crowding the frame.\n- Press [TAB] during the active contract to review its exact qualifications.", 3, 10);
    ...
    {
        AddKnowledgeEntry("quiet_movement", "TECHNIQUE - QUIET MOVEMENT ON SET", "Hold either CTRL key with WASD to walk slowly without footstep sounds, with any equipment or empty hands. CTRL takes priority over SHIFT sprint. Release CTRL to return to normal movement. When holding a camera, CTRL also keeps the existing gentle mouse look and eased movement for smooth shots. Jump landings still make sound.", "Technique", 1, 5);
